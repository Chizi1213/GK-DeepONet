...
class DeepONetCartesianProd(nn.Module):
    ...
    def forward(self, inputs, training=False):
        ...
        sigma = 0.4                            # Hyperparameter, adjustable
        ...
        # - Branch side: Use mean or max similarity as global context
        global_sim = K.mean(dim=1)  # (B, 1)
        x_func3 = torch.cat([x_func2, global_sim], dim=-1)  # (B, H+1)

        # - Trunk side: Use similarity as local context
...
# Trunk back part: input shape: (b, N_Node, HIDDEN)
...
# Initialize DeepONetCartesianProd model
...
round = 'r5'                      # r6 is Einstein summation, r5 is Gaussian kernel
    ...
    for morph in ['100']:
        ...
        for i in test_indices:
            ...
            for j in range(len(Geom)): 
                ...
                plots = [
                    ("field_true", clim_true, "FEM"),
                    ("field_pred", clim_pred, "GK-DeepONet"),
                    ("field_ae",   clim_ae,   "GK-DeepONet_Ae")
                ]
                ...
                for scalar_name, clim, title_suffix in plots:
                    ...
                    try:
                        ...
                        # Image title uses original title_suffix (e.g., FEM, GK-DeepONet)
